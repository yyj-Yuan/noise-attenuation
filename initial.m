function yc=initial(Vmin,Vmax,dv,data,OFF,dt,i0,j0,M,N,p)
    %  initial: calculate the initial noise model
    %
    ...
    for k=0:Nv
        Vk(k+1)=Vmin+k*dv; %the intermediate velocity values
    end
    
    % extract the sample point w from input data, and the boundaries of seismic data are specially processed
    w=zeros(N,M,Nv+1);
    for i=1:N
        for j=1:M
            for k=1:Nv+1
    ...
    % obtain the initial model of coherent noise
    y=U11';
    ...
    % the typical values c1 and c2 are determined experimentally by trial-and-error
    c1=0.6;
    c2=0.8;
    
    % define the weighting coefficient
    if R<=c1
        c=0;
    elseif c1<R&&R<c2
        c=(R-c1)/(c2-c1);
    ...
    % obtain the corrected noise model
    yc=y.*c;
end